...
//        id_btn_refresh.setVisibility(View.GONE);
        ...
        if (savedInstanceState == null) {
            // delete all pending cart yesterday  function call
            DeleteExpirePendingCart();
    ...
    // delete all pending cart yesterday
    private void DeleteExpirePendingCart() {
        Calendar form = Calendar.getInstance();
        form.add(Calendar.DATE,-100);
        Calendar to = Calendar.getInstance();
        to.add(Calendar.DATE,-1);
        List<Integer> list= db.getAllExpireCart(form.getTimeInMillis(),to.getTimeInMillis());
        int size = list.size();
        for(int i=0;i<size;i++)
        {
            db.DeleteExpireCart(list.get(i));
        }
        db.ALLDeleteExpirePendingCart(form.getTimeInMillis(),to.getTimeInMillis());
    ...
    public void onBackPressed() {
        if (drawer.isDrawerOpen(GravityCompat.START)) {
            ...
            if (doubleBackToExitPressedOnce) {
                new AlertDialog.Builder(this,R.style.ThemeOverlay_MaterialComponents_MaterialAlertDialog_Background)
                        .setTitle("Close App")
